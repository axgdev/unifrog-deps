...
int renderer_init(void)
{
  ...
  GPUIF_LOG("memset done");

  GPUIF_LOG("Setting gpu_unai.vram to %p", (void*)gpu.vram);
  ...
  GPUIF_LOG("Setting TextureWindow...");
  ...
  GPUIF_LOG("Building s_invTable (INT_DIV_MULTINV)...");
  // s_invTable
  ...
  GPUIF_LOG("Calling SetupLightLUT()...");
  SetupLightLUT();
  GPUIF_LOG("SetupLightLUT() done");

  GPUIF_LOG("Calling SetupDitheringConstants()...");
  SetupDitheringConstants();
  GPUIF_LOG("SetupDitheringConstants() done");

  GPUIF_LOG("renderer_init() complete");
  return 0;
}
