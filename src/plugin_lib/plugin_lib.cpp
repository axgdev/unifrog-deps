...
#include "psxcommon.h"
#include "port.h"  /* For SF2000 usleep stub */
...
/* QPSX_041: Skip frame limiting during frameskip (set by retro_run) */
...
void pl_frame_limit(void)
{
	...
	/* QPSX_041: Also skip usleep during frameskip (when skip_video_output is set)
	 * Config.FrameLimit should be 0 in libretro mode, but check both for safety.
	 */
	if (Config.FrameLimit && !skip_video_output && (diff > pl_data.frame_interval)) {
